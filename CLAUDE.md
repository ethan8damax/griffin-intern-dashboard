...
- `src/components/intern-dashboard.tsx` — the entire app: top nav (`TopNav`, with dropdown menus for the Planning/Performance/Resources groups defined in `dashboard-data.ts`), and one component per tab (Dashboard, Scorecard, Journal, Goals, Projects, Workload, Weekly Priorities, Weekly Reflections, 360 Feedback, Review Summaries, Reference, Saved — plus Profile, which has no nav entry and is only reached by clicking a person's avatar chip in the top bar, see below). Also owns PDF export (opens a print window) and the standalone review-link flow (`?review=<token>` query param renders a bare review form, no header/nav).
...
- **Projects / Workload tabs**: `AppState.projects` (`owner` field is the intern's name) drives both — Projects is the editable CRUD surface grouped by intern, Workload is a derived, read-mostly view per intern (project checklist, unique `assignedBy` list, soonest non-Complete due dates, "Blocked?" from any `status === "Blocked"`) plus one manually-set field, `AppState.workloads[name]`, for "Current Capacity" — there's no formula behind that number (e.g. derived from project count/estimated time) since none was specified; it's just a slider like the rest of the dashboard's manual status fields. Same joined-by-name caveat as Profile: renaming a person would orphan their projects/workload entry.
- **Goal progress**: `Objective.progress` (0–100) is a manually-set slider, not derived from key-result completion or linked project status — no attempt yet to auto-roll-up progress from Projects/Priorities into Goals.
